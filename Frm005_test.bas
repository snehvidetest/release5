...
Private Function CheckFields(sheet As String)
    ...
    If (sheet = "SpmSvar") Then
        result = findPreviousAns(findTopSpm("A"), "5", 1, 1)  'ThisWorkbook.Sheets(sheet).Range("D13").Text
    Else
...
Private Function DataIsSaved(sheet As String, cell As String)
    ...
    Else
        Select Case parameters("testParameter")
            Case "optionButton1"
                Call writeSpmSvar("5", "", "", "", 6) 'ThisWorkbook.Sheets(sheet).Range(cell).Value = ""
                ShowFunc (formName)
                result = CStr(frm005.OptionButton1.Value)
            Case "optionButton2"
                Call writeSpmSvar("5", "", "", "", 6) 'ThisWorkbook.Sheets(sheet).Range(cell).Value = ""
