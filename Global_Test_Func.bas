...
wb.Sheets("SpmSvar").Range("A2:I111") = ""
...
Function CheckPrintsInAllSheets(spmCells As Scripting.Dictionary, popCells As Scripting.Dictionary, rulCells As Scripting.Dictionary, groCells As Scripting.Dictionary) As String
...
Function CheckPrintsInSpecificSheet(actChangedCells As Scripting.Dictionary, expChangedCells As Scripting.Dictionary) As String
    
    
    If (expChangedCells.Count <> 0) Then
        For Each element In expChangedCells
            If actChangedCells.exists(element) Then
                If expChangedCells(element) = actChangedCells(element) Then
                    actChangedCells.Remove (element)
                    expChangedCells.Remove (element)
                End If
            End If
        Next element
    End If
    
    If (actChangedCells.Count = 0 And expChangedCells.Count = 0) Then
    ...
    Else
        ...
        i = 0
        ReDim temp(expChangedCells.Count + actChangedCells.Count + 2)
        ...
        If (expChangedCells.Count <> 0) Then
            ...
            For Each element In expChangedCells
                i = i + 1
                temp(i) = element + ": " + expChangedCells(element)
            Next element
            i = i + 1
        End If
        If (actChangedCells.Count <> 0) Then
            temp(i) = vbLf + "Act: "
            For Each element In actChangedCells
                i = i + 1
                temp(i) = element + ": " + actChangedCells(element)
            Next element
        End If
...
'myString = CStr(s1)
lookupRow = findRow(500, s1, 0, "Spm_Lookup")
spmRow = findRow(500, s1)
If spmRow <> 0 Then
    Main_Test_Form.spmCells.Add "A" & CStr(spmRow), CStr(ThisWorkbook.Sheets("Spm_Lookup").Cells(lookupRow, 1).Value)
    Main_Test_Form.spmCells.Add "B" & CStr(spmRow), CStr(ThisWorkbook.Sheets("Spm_Lookup").Cells(lookupRow, 2).Value)
    Main_Test_Form.spmCells.Add "C" & CStr(spmRow), ans 'Frm002_test.parameters(ThisWorkbook.Sheets("Spm_Lookup").Cells(lookupRow, 5).Value)
    Main_Test_Form.spmCells.Add "D" & CStr(spmRow), ans2 'Frm002_test.parameters(ThisWorkbook.Sheets("Spm_Lookup").Cells(lookupRow, 6).Value)
End If
End Function
